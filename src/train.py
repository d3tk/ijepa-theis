...
def main(args, resume_preempt=False):
    ...
    if rank == 0:
        # Customize your wandb.init() call as needed
        wandb.init(
            project="I-JEPA-RUNS",
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        train_loader_tqdm = tqdm(
            ...
            total=len(unsupervised_loader),
            desc=f"Epoch {epoch+1}/{num_epochs}",
            disable=(rank != 0)  # show only on main process
        )

        for itr, (udata, masks_enc, masks_pred) in train_loader_tqdm:
            ...
            def train_step():
                ...
                with torch.amp.autocast(dtype=torch.bfloat16, enabled=use_bfloat16, device_type='cuda'):
                    h = forward_target()
                    z = forward_context()
                    loss = loss_fn(z, h)
                ...
                train_loader_tqdm.set_postfix(
                    {
                        ...
                        "lr": f"{_new_lr:.6f}",
                        "mem": f"{torch.cuda.max_memory_allocated() / 1024 ** 3:.2f}G",
                    }
                )
            ...
            def log_stats():
                ...
                if rank == 0 and (itr % log_freq == 0):
                    wandb.log({
                        ...
                        # optionally log grad stats:
                        "grad/first_layer": grad_stats.first_layer if grad_stats else None,
                        "grad/last_layer": grad_stats.last_layer if grad_stats else None,
                        "grad/min": grad_stats.min if grad_stats else None,
                        "grad/max": grad_stats.max if grad_stats else None
                    })
